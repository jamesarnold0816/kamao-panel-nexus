...
const ResellerIntroduction = () => {
  const { user } = useUser();
  const { requestPlanUpgrade, planUpgradeRequests } = useCart();
  ...
  const handleUpgradePlan = (plan: 'free' | 'basic' | 'vip') => {
    ...
    if (
      (user.plan === 'vip' && (plan === 'basic' || plan === 'free')) ||
      (user.plan === 'basic' && plan === 'free')
    ) {
      toast.error("You can't downgrade your plan. Please contact support if needed.");
      return;
    }
    ...
    if (hasPendingRequest) {
      toast.warning("You already have a pending plan upgrade request.");
      return;
    }

    // Create a plan upgrade request
    requestPlanUpgrade(
      (user.plan as 'free' | 'basic' | 'vip') || 'free',
      plan
    );
    toast.success(`Upgrade request to ${plan.toUpperCase()} plan submitted for approval.`);
  };
      ...
      <div className="space-y-8">
        ...
        {hasPendingRequest && (
          <div className="bg-yellow-50 border border-yellow-200 rounded-md p-4 mb-4 flex items-center">
            ...
            <div>
              <h3 className="text-sm font-medium text-yellow-800">Plan Upgrade Pending</h3>
              <p className="text-sm text-yellow-700 mt-1">
                Your plan upgrade request is being reviewed by our admin team. You'll be notified once it's approved.
              </p>
            </div>
          </div>
        )}
          ...
          <TabsContent value="plans" className="space-y-6">
            ...
            <div className="grid md:grid-cols-3 gap-6">
              <PlanCard
                ...
                disabled={user?.plan === 'free' || hasPendingRequest}
              />
              <PlanCard
                title="Basic Plan"
                ...
                disabled={user?.plan === 'basic' || hasPendingRequest || user?.plan === 'vip'}
                popular={true}
              />
              <PlanCard
                title="VIP Plan"
                ...
                disabled={user?.plan === 'vip' || hasPendingRequest}
              />
            </div>
            ...
            <div className="bg-gray-100 p-6 rounded-lg mt-8">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <p className="text-sm text-gray-600">Yes, you can request a plan upgrade at any time. Upgrades require admin approval.</p>
                </div>
                <div>
                  <h4 className="font-medium">What happens when I upgrade?</h4>
                  <p className="text-sm text-gray-600">
                    After your upgrade request is approved by the admin, you'll gain access to all the products and features available in your new plan.
                  </p>
                </div>
                <div>
                  <h4 className="font-medium">How long does approval take?</h4>
                  <p className="text-sm text-gray-600">
                    Plan upgrade requests are typically reviewed within 24-48 hours by our admin team.
                  </p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600">
                    Currently, we don't support self-service downgrading. Please contact support if you need to downgrade.
                  </p>
                </div>
              </div>
            </div>
