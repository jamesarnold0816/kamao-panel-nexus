...
const ResellerProducts = () => {
  const { filteredProducts, categories, fetchProducts, fetchCategories, loading } = useProducts();
  const { user } = useUser();
  ...
  useEffect(() => {
    ...
    fetchProducts(selectedCategory, searchQuery, userPlan);
  }, [searchQuery, selectedCategory, user?.plan]);
