...
const ResellerDashboard = () => {
  ...
  const userOrders = orders.filter(order => order.resellerId === user?.id);
  
  // Get products based on user's plan
  const availableProducts = products.filter(product => {
    if (!user) return product.accessPlan === 'free';
    
    switch (user.plan) {
      case 'free':
        return product.accessPlan === 'free';
      case 'basic':
        return product.accessPlan === 'free' || product.accessPlan === 'basic';
      case 'vip':
        return true;
      default:
        return product.accessPlan === 'free';
                ...
                <CardContent className="p-6">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className={cn(
                      ...
                      product.accessPlan === 'free' 
                        ? 'bg-gray-100 text-gray-800' 
                        : product.accessPlan === 'basic'
                        ? 'bg-kamao-purple-light text-kamao-purple'
                        : 'bg-purple-100 text-purple-800'
                    )}>
                      {product.accessPlan.toUpperCase()}
                    </span>
                  </div>
                </CardContent>
